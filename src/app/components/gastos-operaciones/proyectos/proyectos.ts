...
import { switchMap, map, catchError, finalize, concatMap, tap, toArray } from 'rxjs/operators';
  ...
    imports: [CommonModule, NgIf, FormsModule, ConfirmacionComponent, OkComponent, ErrorComponent, ModuloComponent, ItemsGastoOperacion, ReportesPDf],
  ...
    
    // -- Parámetros inicializados a null para que el placeholder sea visible --
    ...
    private cargarItemsProyecto(idGeneral: number): void {
      ...
      this.servicios.getProyectoCompleto(idGeneral).subscribe({
        ...
        error: (err) => {
          ...
          this.items = [];
          this.mostrarMensaje('error', 'Error al cargar módulos e ítems del proyecto.');
        },
      });
    }
  ...
            // Mostrar mensaje de éxito
            this.mostrarMensaje('exito', 'Parámetros actualizados correctamente.');
          },
          error: (err) => {
  ...
    }
    ...
    duplicarProyecto(proyecto: Proyecto, event: Event): void {
      ...
      this.servicios.createIdentificadorGeneral(proyectoDuplicadoPayload).pipe(
        ...
        if (resultado) {
          setTimeout(() => {
            this.mostrarMensaje('exito', 'Proyecto duplicado correctamente con módulos, gastos e ítems.');
          }, 5000);
        }
      });
    }
  ...
        this.gastos_generales = this.proyectoSeleccionado.gastos_generales ?? null;
        this.iva_tasa_nominal = this.proyectoSeleccionado.iva_tasa_nominal ?? null;
