...
export class GastosOperacionesComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    // Inicializar modales sólo si existen en el DOM (evita pasar null a bootstrap)
    ...
    // El id en el HTML es 'modalProyectoParametros'
    ...
    this.modalMover = modalMoverEl ? new bootstrap.Modal(modalMoverEl) : null;
  ...
  ngAfterViewInit() {
    // Asegurar que no se intenta crear dos veces el mismo modal y que el elemento existe
    const modalMoverEl = document.getElementById('modalMover');
    if (modalMoverEl && !this.modalMover) {
      this.modalMover = new bootstrap.Modal(modalMoverEl);
    }
  ...
  cargarUnidadesGasto(): void {
    this.servicios.getUnidadesGastoOperacion().subscribe({
      next: (unidades: string[]) => {
        ...
        // Inicializar arrays relacionados
        this.unidadesFiltradas = this.items.map(() => [...this.unidadesUsadas]);
        this.mostrarLista = this.items.map(() => false);
      },
      error: (err) => {
        // Manejo silencioso del error (sin console.log)
        this.unidadesUsadas = []; // Inicializar como array vacío
        this.unidadesFiltradas = this.items.map(() => []);
        this.mostrarLista = this.items.map(() => false);
      },
    });
  }
  ...
  private asignarProyecto(proyecto: Proyecto | null) {
    ...
    // Carga optimizada en una sola operación
  ...
  private cargarDatosProyecto(idGeneral: number): void {
    ...
    this.cargando = true;
    const modulosTemporales = this.items.filter((it) => it.tipo === 'modulo');
    ...
    this.servicios.getProyectoCompleto(idGeneral).subscribe({
      ...
      error: (err) => {
        ...
        this.mostrarMensaje('error', 'Error al cargar datos del proyecto');
        this.resetearEstadoItems();
      },
  ...
  private aplicarTotales(
    totales: { [key: string]: number } | null | undefined
  ): void {
    if (!totales) return;
    this.items.forEach((item) => {
      if (item.tipo === 'gasto' && item.id != null) {
        const keyStr = String(item.id);
        const total =
          (totales as any)[keyStr] ?? (totales as any)[item.id] ?? undefined;
        if (total !== undefined) {
    ...
    this.servicios
      ...
      .subscribe({
        next: (resp) => {
          ...
          this.mostrarModalExito('Parámetros actualizados correctamente');
        },
        error: (err) => {
          this.mostrarModalError(
  ...
  cerrarModalParametros() {
    ...
    // ❌ Quitar foco de cualquier botón dentro del modal
    const btnFoco = modalEl.querySelector('button:focus') as HTMLElement;
    if (btnFoco) btnFoco.blur();

    // ✅ Usar Bootstrap para cerrar modal correctamente
  ...
  cargarGastos(idGeneral: number): void {
    ...
    forkJoin({
      modulos: this.servicios.getModulosPorProyecto(idGeneral),
      gastos: this.servicios.getGastoOperacionID(idGeneral),
      totales: this.servicios.getGastosGeneralesPorProyecto(idGeneral),
    }).subscribe({
      next: ({ modulos, gastos, totales }) => {
        // Procesar módulos
        this.modulos = modulos || [];
        ...
        // Aplicar totales de manera eficiente
  ...
  private finalizarCargaItems(): void {
    ...
    this.items.forEach((item) => {
      if (item.unidad) this.agregarUnidadSiNoExiste(item.unidad); // Cambiado de unidad_medida a unidad
      if (item.descripcion) this.agregarDescripcionSiNoExiste(item.descripcion);
    });
    this.cdr.detectChanges();
  }
  private finalizarCarga(): void {
    // Reusa la lógica existente para preparar arrays auxiliares
    this.finalizarCargaItems();
    // Forzar render y limpiar flags de carga
    this.cdr.detectChanges();
    this.cargando = false;
  }
  agregarItem(): void {
  ...
  get total(): number {
    const total = this.items
      .filter((item) => item.tipo === 'gasto' && item.id) // Ignora temporales
      .reduce(
        (acc, item) =>
